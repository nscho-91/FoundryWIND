...
{
    public partial class UI_ManualJob_MSB : Form, UI_ManualJob_Mom
    {
        public string m_id;
        int m_nLoadPort = 0;
        bool m_bMapDataUse = false;
        bool m_bSelectDone = false; // ing 170406
        string m_strPath; 
        protected Log m_log;
        ...
        private void UI_ManualJob_MSB_Shown(object sender, EventArgs e)
        {
            tbLotID.Focus();
            tbLotID.SelectAll();
        }
        ...
        {
            bool bRst = false;
            m_recipe.JobOpen(m_recipe.m_strFile); // ing 170406
            ...
            if (m_infoCarrier.m_strCarrierID == "")
            {
                MessageBox.Show("Please Write Down Carrier ID.");
                this.DialogResult = System.Windows.Forms.DialogResult.None;
                return;
            }
            if (m_infoCarrier.m_strLotID == "")
            {
                MessageBox.Show("Please Write Down Lot ID.");
                this.DialogResult = System.Windows.Forms.DialogResult.None;
                return;
            }
        ...
        public void RecipeFileSearch()
        {
            ...
            try
            {
                ...
                {
                    m_recipe.JobOpen(file.FullName, false);
                    ...
                    if (m_recipe.m_wafer.m_eSize == m_infoCarrier.m_wafer.m_eSize) 
                    {
                        ...
                        if (textBoxNameFilter.Text == "" || m_recipe.m_sRecipe.ToLower().Contains(textBoxNameFilter.Text.ToLower()) == true)
                        {
            ...
            if (m_recipe.m_strFile != "")
            {
                ...
                m_recipe.RunGrid(eGrid.eUpdate);
        ...
        private void listViewRecipe_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            ...
            m_recipe.JobOpen(m_recipe.m_strFile, false);
            m_recipe.RunGrid(eGrid.eInit); 
        ...
        {
            string strFile;
            strFile = m_auto.ClassRecipe().GetFileName(m_auto.ClassXGem().m_XGem300Process[m_nLoadPort].m_sRecipeID);
            m_recipe.JobOpen(strFile, true);
            
            
            /*for (int n = 0; n < m_waferselect.Length; n++) {
                if (m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n].IsExist() == true) {
                    m_waferselect[n].chkBox.Checked = true;
                    m_recipe.SetInfoWafer(m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n]);
                }
            }*/

            //Insp Mode 추가 
            switch (m_auto.ClassRecipe().m_eInspMode) {
                case Recipe_Mom.eInspMode.FullSlot:
                    for (int n = 0; n < m_waferselect.Length; n++) {
                        if (m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n].IsExist() == true) {
                            m_waferselect[n].chkBox.Checked = true;
                            m_recipe.SetInfoWafer(m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n], null);
                        }
                    }
                    break;
                case Recipe_Mom.eInspMode.CountinuousSlot:
                    if (m_auto.ClassRecipe().m_nInspCount == 0) {
                        m_log.Popup("Insp Count 0");
                        break;
                    }
                    if (m_auto.ClassRecipe().m_eInspDIr == Recipe_Mom.eInspDir.Bottom) {
                        int nSelect = 0;
                        for (int n = 0; n < m_waferselect.Length; n++) {
                            if (m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n].IsExist() == true) {
                                m_waferselect[n].chkBox.Checked = true;
                                m_recipe.SetInfoWafer(m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n], null);
                                nSelect++;
                                if (nSelect == m_auto.ClassRecipe().m_nInspCount)
                                    break;
                            }
                        }
                    }
                        ...
                        int nSelect = 0;
                        for (int n = m_waferselect.Length -1; n >= 0; n--) {
                            if (m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n].IsExist() == true) {
                                m_waferselect[n].chkBox.Checked = true;
                                m_recipe.SetInfoWafer(m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n], null);
                                nSelect++;
                    ...
                    break;
                case Recipe_Mom.eInspMode.SelectedSlot:
                    for (int n = 0; n < m_waferselect.Length; n++) {
                        if (m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n].IsExist() == true && m_auto.ClassRecipe().m_aInspSelect[n] == true) {
                            m_waferselect[n].chkBox.Checked = true;
                            m_recipe.SetInfoWafer(m_handler.ClassCarrier(m_nLoadPort).m_infoWafer[n], null);
                        }
                    }
            ...
            m_infoCarrier.m_strLotID = m_auto.ClassXGem().m_XGem300Process[m_nLoadPort].m_sJobID;
        ...
        private void panelWaferInfo_Layout(object sender, LayoutEventArgs e)
        {
            //RecipeFileSearch();            
        }
