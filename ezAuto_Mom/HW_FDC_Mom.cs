...
namespace ezAutoMom
{
    public partial class HW_FDC_Mom : Form
    {
        ...
        protected void RunGrid(eGrid eMode)
        {
            ...
            for (int j = 0; j < m_handler.m_nFDCModuleNum; j++) 
            {
                ...
                m_grid.Set(ref m_FDCModule[j].m_nSV, j.ToString("FDC 0"), "SV", "Gem SV"); 
                m_grid.Set(ref m_FDCModule[j].m_bFDCError, j.ToString("FDC 0"), "Error", "Select FDC Error State");
        ...
        void m_rs232_CallMsgRcv()
        {
            int nRead;
            Thread.Sleep(100);
            nRead = m_rs232.Read(m_aBuf, BUF_SIZE, 0, false);     
            if (nRead != 9) return; 
    ...
    public class FDCModule
    {
        ...
        public bool m_bFDCError = false;
        public string m_sFDCError = ""; 
